...
use crate::{
    application::services::health_service::HealthService,
    infrastructure::repositories::in_memory_health_repository::InMemoryHealthRepository,
    presentation::router::router::configure_routes, state::AppState,
};
use actix_web::{App, HttpServer};

pub async fn run() -> std::io::Result<()> {
    let app_config = config::AppConfig::load("api-gateway").expect("Failed to Load Configuration");
    let health_repository = Arc::new(InMemoryHealthRepository::new(app_config.app_name.clone()));
