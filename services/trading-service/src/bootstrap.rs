...
use crate::{
    ...
    infrastructure::{
        ...
        grpc::{market_client::MarketGrpcClient, account_client::AccountGrpcClient},
        repositories::{
...
pub async fn bootstrap() -> Result<(AppState, impl std::future::Future<Output = Result<(), tonic::transport::Error>>)> {
    ...
    println!("Market cache initialized with {} markets", market_cache.len().await);
