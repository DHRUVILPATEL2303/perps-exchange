use crate::{
    ...
    infrastructure::{
        ...
        kafka::producer::KafkaOrderEvent,
    },
};
use proto::trading::{
...
impl GrpcTradingService for TradingGrpcService {
    async fn place_order(
        ...
        let payload =
            serde_json::to_vec(&kafka_event).map_err(|e| Status::internal(e.to_string()))?;

        if let Err(publish_err) = self.order_publisher.publish(&payload).await {
            tracing::error!(
                "Failed to publish order {} to Aeron: {}. Executing SAGA compensating rollback...",
                order_id,
                publish_err
            );
            ...
            {
                let mut acc_client = self.account_client.lock().await;
                if let Err(rollback_err) = acc_client
                    .release_margin(
                        req.user_id.clone(),
                        check_res.required_margin.clone(),
                        order_id.to_string(),
